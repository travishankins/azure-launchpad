...
```hcl
# Generated by the Azure Launchpad wizard (or hand-rolled)
...
terraform init \
  ...
  -backend-config="key=foundation.full.multi.tfstate"

terraform workspace select -or-create full-multi
...
The shipped wrapper [`scripts/deploy-multi-sub.sh`](https://github.com/travishankins/azure-launchpad/blob/main/scripts/deploy-multi-sub.sh) runs the whole sequence:
...
`--scenario` accepts `baseline`, `firewall`, `vpn`, or `full`. Defaults to `baseline`.
